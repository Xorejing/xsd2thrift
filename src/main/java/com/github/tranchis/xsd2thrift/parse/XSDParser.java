...
public class XSDParser {
		...
		try {
			...
			if (!ss.isEmpty()) {
				...
				if (requiredTypes.isEmpty()) {
					...
					throw new Xsd2ThriftException("Source schema contains references missing types.");
				}
			}
		} catch (IOException e) {
			throw new Xsd2ThriftException(e);
	...
	private void write(Struct st, XSAttributeDecl decl, boolean goingup) {
		if (decl.getType().isRestriction()
		        && (decl.getType().getName() == null || !basicTypes.contains(decl.getType().getName()))) {
			...
			st.addField(decl.getName(), decl.getType().asList().getItemType().getName(), goingup, true, null,
			        xsdMapping);
		} else {
	...
	private void write(Struct st, XSAttGroupDecl attGroup, boolean goingup) {
		...
		while (it.hasNext()) {
			...
			if (decl.getType().getName() == null) {
				if (decl.getType().isRestriction()) {
					String typeName = processSimpleType(decl.getType(), decl.getName());
					st.addField(decl.getName(), typeName, (goingup && att.isRequired()), false, decl.getFixedValue(),
					        xsdMapping);
				}
			} else {
			...
			for (int i = 0; i < ps.length; i++) {
				...
				if (term.isModelGroup()) {
					...
					if (term.asElementDecl().getType().getName() == null) {
						final String typeName = processType(term.asElementDecl().getType(),
						        term.asElementDecl().getName(), xss);
						st.addField(term.asElementDecl().getName(), typeName,
						        (goingup && p.getMinOccurs().intValue() != 0), p.getMaxOccurs().intValue() != 1,
